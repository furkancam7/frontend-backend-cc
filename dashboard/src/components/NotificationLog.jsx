...
const formatAccuracy = (acc, t) => {
  if (acc == null) return t('notificationLog.na');
  const val = Number(acc);
  if (Number.isNaN(val)) return t('notificationLog.na');
  return `${(val > 1 ? val : val * 100).toFixed(1)}%`;
};

const getTimeAgo = (timestamp, t, locale) => {
  if (!timestamp) return t('notificationLog.unknown');

  try {
    const now = Date.now();
    const time = new Date(timestamp).getTime();
    if (Number.isNaN(time)) return t('notificationLog.invalid');

    const diff = Math.max(0, Math.floor((now - time) / 1000));
    if (diff < 5) return t('notificationLog.justNow');
    if (diff < 60) return t('notificationLog.secondsAgo', { count: diff });
    if (diff < 3600) return t('notificationLog.minutesAgo', { count: Math.floor(diff / 60) });
    if (diff < 86400) return t('notificationLog.hoursAgo', { count: Math.floor(diff / 3600) });

    return new Date(time).toLocaleDateString(locale);
  } catch {
    return t('notificationLog.unknown');
  }
};

const LogItem = memo(({ log, category, onItemClick, t, locale }) => {
  const timeAgo = getTimeAgo(log.timestamp, t, locale);
  const [imgError, setImgError] = useState(false);
  const localizedClass = useMemo(() => localizeDetectionClassName(log.class, t), [log.class, t]);
    ...
    >
      <div className="w-11 h-11 rounded-lg overflow-hidden shrink-0 bg-black flex items-center justify-center border border-gray-900" style={{ borderColor: `${category?.color}30` }}>
        {!imgError && log.crop_id ? (
          <img
            ...
            alt={`${log.class} ${t('notificationLog.title')}`}
            loading="lazy"
      ...
      <div className="flex-1 min-w-0 overflow-hidden">
        ...
        <div className="text-xs text-zinc-200 truncate">
          {localizedClass} <span className="text-zinc-500 mx-1">-</span> {formatAccuracy(log.accuracy, t)}
        </div>
        <div className="text-[10px] text-zinc-500 flex items-center gap-2 mt-0.5 truncate">
          <span className={`truncate ${log.device_id ? 'text-cyan-400' : 'text-zinc-500'}`}>
            {log.device_id || t('notificationLog.unknown')}
          </span>
...
const NotificationLog = ({ logs = [], onItemClick, handleClearLogs }) => {
  const { t, i18n } = useUiTranslation(['notificationLog']);
  ...
  const onSafeClear = useCallback(() => {
    if (typeof window !== 'undefined' && handleClearLogs) {
      if (window.confirm(t('notificationLog.clearConfirm'))) {
        handleClearLogs();
      }
    }
  }, [handleClearLogs, t]);
    ...
    <div className="flex flex-col h-full bg-black text-white">
      <div className="px-4 py-3 border-b border-gray-900 flex items-center justify-between shrink-0">
        <div className="flex items-center gap-3">
          ...
          <div>
            <span className="font-bold text-xs uppercase tracking-wider text-white">{t('notificationLog.title')}</span>
        ...
        <button
          ...
          aria-label={t('notificationLog.clearAllAria')}
        >
      ...
      <div className="flex-1 overflow-y-auto p-3 scrollbar-thin scrollbar-thumb-gray-900 bg-black">
        {filteredLogs.length === 0 ? (
          <div className="flex flex-col items-center justify-center h-[200px] text-gray-700">
            ...
            <span className="text-xs text-gray-600">{t('notificationLog.noDetectionsLogged')}</span>
          </div>
        ) : (
